     ...
     <div className='w-full max-w-[500px] h-[500px] bg-white rounded-lg shadow-gray-400 shadow-lg flex flex-col gap-[30px]'>
        ...
        <form className='w-full flex flex-col gap-[20px] items-center' onSubmit={handleSignUp}>
            ...
            <div className='w-[90%] h-[50px] border-2 border-[#20c7ff] overflow-hidden rounded-lg shadow-gray-200 shadow-lg relative'>
                <input type={`${show?"text":"password"}`} placeholder='password' className='w-full h-full outline-none px-[20px] py-[10px] bg-[white] text-gray-700 text-[19px] appearance-none'
...
{error && <p className='text-red-500'>{error}</p>}
            ...
            <p className='cursor-pointer' onClick={()=>navigate("/login")}>Already Have An Account ? <span className='text-[#20c7ff]'>Log In</span></p>
        </form>
        </div>
